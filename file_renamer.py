...
# The images have to be sorted first by the level (if there is more than 1) and the variants across all levels need to have the same order.
# For example: level1_variation1, level1_variation2, level2_variation1, level2_variation2 (in this case, there would be 2 levels and 2 image variants per level)
...
#------------------------------------------- Modify this ---------------------------------------------
    ...
    # Type the name of the variants the levels will have, in the order they are sorted
    variants = ["Sunny","Night","Rain","Snow"]
...
def rename(folder_path,variants,sorted_by_date,reverse_order,output_path):
    ...
    for filename in filenames:
        new_filename = f"{variants[variation_count]}{level_count}"+os.path.splitext(filename)[1] # Generate the new name of the file: Variation + Level_counter
        ...
        # Add to the variant/level counters
        variation_count += 1 
...
if __name__ == "__main__":
    rename(folder_path,variants,sorted_by_date,reverse_order,output_path)